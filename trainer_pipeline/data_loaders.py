...
AIRCRAFT_IMAGES_DIR = Path("data/benchmark-3/FGVC-Aircraft/fgvc-aircraft-2013b/fgvc-aircraft-2013b/data/images")

# Stanford-Cars
CARS_METADATA_PATH = Path("data/benchmark-3/Stanford-Cars/car_devkit/devkit/cars_meta.mat")
CARS_TRAIN_ANNOS_PATH = Path("data/benchmark-3/Stanford-Cars/car_devkit/devkit/cars_train_annos.mat")
CARS_TEST_ANNOS_PATH = Path("data/benchmark-3/Stanford-Cars/car_devkit/devkit/cars_test_annos_withlabels.mat")
...
class Cub_200_2011_Dataset:
    ...
    def get_raw_dataset(self) -> DatasetDict:
        if self.download:
            ...
            try:
                ...
                print(f"[{self.data_dir}] 경로에 데이터가 없습니다. 다운로드를 시작합니다...")
    ...
    def set_transform(self) -> Tuple[Callable, Callable]:
        ...
        def apply_test_transform(example: dict) -> dict:
    ...
    def get_dataset(
        ...
        apply_test_transform: Callable,
    ) -> Tuple[Dataset, Dataset]:        
        train_dataset = raw_dataset["train"].with_transform(apply_train_transform)
        test_dataset = raw_dataset["test"].with_transform(apply_test_transform)
...
class CarsDataset(Dataset):
    ...
    def __init__(self, image_dir: Path, train_or_test: str, transform: transforms = None):
        ...
        if self.train_or_test == "train":
            ...
            cars_test_annos_withlabels = scipy.io.loadmat(file_name=CARS_TEST_ANNOS_PATH)                
...
def get_dataloaders(
    ...
    """
    Args:
        dataset_name (str): 데이터셋 이름 ("CUB-200-2011", "FGVC-Aircraft", "Stanford-Cars")
    Returns:
        Tuple[DataLoader, DataLoader, DataLoader]: (train_loader, test_loader, test_loader_shuffle)
    """
    if dataset_name == "CUB-200-2011":
        cub_200_2011_dataset = Cub_200_2011_Dataset(data_dir=CUB_DIR, download=download)
        apply_train_transform, apply_test_transform = cub_200_2011_dataset.set_transform()
        raw_dataset = cub_200_2011_dataset.get_raw_dataset()

        train_dataset, test_dataset = cub_200_2011_dataset.get_dataset(raw_dataset, apply_train_transform, apply_test_transform)
        ...
        test_dataset = AircraftDataset(csv_file="test.csv", transform=test_transform)
    ...
    test_loader_shuffle = DataLoader(
        dataset=test_dataset,
        batch_size=batch_size,
        shuffle=True,
        num_workers=4,
    ...
    print(f"테스트용 셔플 배치 수: {len(test_loader_shuffle)}")
